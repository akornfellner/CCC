...
fn main() {
    for i in 0..=5 {
        let input = fs::read_to_string(format!("input/level5_{}.in", i)).unwrap();
        fs::write(format!("output/level5_{}.out", i), solve(&input)).unwrap();
    }
}
...
fn solve(input: &str) -> String {
    ...
    let (_games_count, players_count, size) = (
