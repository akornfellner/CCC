...
fn main() {
    for i in 0..=5 {
        let input = fs::read_to_string(format!("input/level3_{}.in", i)).unwrap();
        fs::write(format!("output/level3_{}.out", i), solve(&input)).unwrap();
    }
}
...
fn solve(input: &str) -> String {
    ...
    let second = lines[1].split_whitespace().collect::<Vec<&str>>();
    ...
    for line in lines.iter().skip(2) {
        ...
        if s1 > s2 {
            *players.entry(p1).or_insert(0) += win;
            *players.entry(p2).or_insert(0) -= lose;
        } else {
            *players.entry(p1).or_insert(0) -= lose;
            *players.entry(p2).or_insert(0) += win;
        }
    }
